...
function buildPagesRaw(
    ...
    while (i < photos.length) {
        ...
        // 펼침 모드에서도 유지한다: 빼면 사진 적은 책에만 6장 모자이크가 없어져 배치 언어가 달라진다.
...
// ⚠️ 두 가지를 지킨다:
//   1) 1장짜리 풀([1] 등)은 쓰지 않는다 — 전 페이지가 사진 1장이 되어 콜라주가 아니라
//      낱장 모음처럼 보인다(히어로/미니멀이 번갈아 나와 들쭉날쭉).
//   2) 4장 배치를 빼지 않는다 — 빼면 사진이 적은 책만 2~3장 일색이 되어
//      기존 앨범과 배치 언어가 달라진다. 6장 모자이크도 펼침 모드에서 그대로 허용.
//   (가장 성긴 풀이 선택되는 구조라, 4를 모든 풀에 넣어야 사진 적은 책에도 4장 배치가 나온다)
const SPREAD_POOLS: number[][] = [[3, 3, 4], [2, 3, 4], [2, 2, 3, 4], [2, 2, 2, 3, 4]];
